...
def parar_tarefa_view(request, pk):
    registro = get_object_or_404(Registro, id=pk)

    if request.method == 'POST':
        # Registra a hora de pausa se ainda não estiver pausado
        if not registro.hora_de_pausa:
            registro.hora_de_pausa = datetime.now().time()
        else:
            # Calcula o tempo pausado e zera a hora de pausa
            ...
            registro.hora_de_pausa = None  # Define como "não pausado"

        registro.save()
        return redirect('index')  # Redireciona para a tela inicial

    return render(request, 'pausar.html', {'registro': registro})
